...
#MAP LAYER 3 TO LAYER 1 (GRID LAUNCH AND SELECTED FX ON ONE PAGE)
...
#class BaseJor(Base): #Original
class BaseJor(Base, ControlSurface): #Extend control surface class for ClyphX linking
	...
	def _setup_translations(self):
		...
		controls = [self._pad[6], self._pad[14], self._pad[22], self._pad[30], self._pad_CC[6], self._pad_CC[14], self._pad_CC[22], self._pad_CC[30]]
	...
	def _setup_session_control(self):
		...
		self._session.cliplaunch_layer = AddLayerMode(self._session, Layer(priority = 4, clip_launch_buttons = self._base_grid.submatrix[:6, :], scene_launch_buttons = self._base_grid.submatrix[7:8, :]))
		self._session.overlay_cliplaunch_layer = AddLayerMode(self._session, Layer(priority = 4, clip_launch_buttons = self._base_grid.submatrix[:6, :], scene_launch_buttons = self._base_grid.submatrix[7:8, :]))
		self._session.scenelaunch_layer = AddLayerMode(self._session, Layer(priority = 4, scene_launch_buttons = self._base_grid.submatrix[7:8, :]))
	...
	def _setup_instrument(self):
		...
		self._instrument.audioloop_layer = LayerMode(self._instrument, Layer(priority = 6, loop_selector_matrix = self._base_grid.submatrix[:6, :]))
	...
	def _setup_main_modes(self):
		...
		self._main_modes.add_mode('Clips_shifted', [
				self._mixer,
				self._mixer.volume_layer,
				self._mixer.select_layer,
				self._mixer.channel_controls_layer,
				self._session_ring,
				#self._session_navigation,
				#self._session_navigation.page_navigation_layer,
				self._device_navigator.main_layer, #NEW
				self.clips_layer_sysex,
				self.live_mode_sysex,
				self._translationsJor,
				self._view_controller], groups = ['shifted'], behaviour = self._shift_latching(color = 'MainModes.Clips'), display_string = MODE_DATA['Clips_shifted'])

		self._main_modes.add_mode('Clips', [
				self._mixer,
				#self._mixer.volume_layer,
				self._device, #NEW
				self.device_layer_sysex, #NEW
				self._device.parameters_layer, #NEW
				#self._device_navigator.main_layer,
				self._mixer.select_layer,
				self._session_ring,
				self._session,
				self._session.cliplaunch_layer,
				self._session_navigation,
				self._session_navigation.navigation_layer,
				self.clips_layer_sysex,
				self.live_mode_sysex,
				self._translationsJor,
				self._view_controller], behaviour = self._shift_latching(color = 'MainModes.Clips'), display_string = MODE_DATA['Clips'])
		...
		self._main_modes.add_mode('Device', [
				self._session,
				self._session.scenelaunch_layer,
				self.device_layer_sysex,
				self._modswitcher,
				self._mixer,
				self._mixer.select_layer,
				self._session_ring,
				self._device,
				self._device.parameters_layer,
				self._device.nav_layer,
				self._device_navigator.main_layer,
				self._translationsJor,
				self._view_controller], behaviour = self._shift_latching(color = 'MainModes.Device'), display_string = MODE_DATA['Device'])
		...
		self._main_modes.add_mode('User', [self._session, self._session.scenelaunch_layer, self._translations, self._mixer, self._mixer.select_layer, self.user_layer_sysex, self.user_mode_sysex, self._translationsJor, self._view_controller], behaviour = self._shift_latching(color = 'MainModes.User'), display_string = MODE_DATA['User'])
